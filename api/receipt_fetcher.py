    ...
    for log in logs[::-1]:
        ...
        if event_sig == SWAP_EVENT:
            ...
            if dst == receiver and assetIn == '':
                assetIn = getTokenInfo(emitter)
                amountIn = amount
            elif src == receiver and assetOut == '':
                assetOut = getTokenInfo(emitter)
                amountOut = amount
...
def isApproval(log):
    ...
    data = data2Uint(log['data'])
    print('Approval({}, {}, {}, {})'.format(owner, spender, getTokenInfo(log['address']), data))
...
def get_receipts(tx_hash):
    ...
    response = requests.post(rpc_url, json.dumps(data))
